...
//
// Invoked on a schedule (external cron POSTing this URL with ?token=$HEARTBEAT_TOKEN;
...
// The two DB signals (1 + 2) come from the shared _shared/monitoring.ts module — the
// SAME code the admin `/health` endpoint uses, so the two never drift. Always returns
// a JSON summary.
...
Deno.serve(async (req) => {
  ...
  // Shared DB signals (unsent leads + abuse burst) — same code as admin /health.
  const summary = await collectMonitoring(supabase);
  const problems: string[] = [];

  if (summary.unsentLeads > 0) {
    problems.push(`${summary.unsentLeads} lead(s) ended in the last hour with NO dispatch SMS — Twilio/dispatch may be failing.`);
  }
  if (summary.abusers.length > 0) {
    problems.push(`Possible abuse: ${summary.abusers.map((a) => `${a.phone} (${a.calls} calls/hr)`).join(", ")}. Consider banning via the admin tool.`);
  }

  // Vapi balance (best-effort, usually disabled).
