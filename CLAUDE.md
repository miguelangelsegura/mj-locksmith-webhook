...
- **Leads tab = the outreach call sheet** (the team's shared prospect list, distinct from `clients`). Backed by the `leads` + `lead_activity` tables and the admin routes `GET/POST /leads`, `GET/PATCH/DELETE /leads/:id`, `POST /leads/:id/activity`. Invariants: `POST /leads` **enriches, never overwrites** (a rerun of the outreach skill must not clobber a human's status/owner/notes); `domain` and `phone` are the dedup keys and are UNIQUE — directory/social hosts are rejected as a `domain` by `NON_IDENTITY_DOMAINS` so every Facebook-only shop doesn't collapse into one row; `lead_activity` is **append-only** history; and logging a call goes through the `log_lead_event` SQL function, which moves the lead row and appends its history in ONE transaction — a half-applied save would otherwise count an attempt with no note to show for it, and `attempts` increments in SQL so two people logging calls at once can't lose one.
- **Not yet built** (see [docs/ADMIN-DASHBOARD-SPEC.md](docs/ADMIN-DASHBOARD-SPEC.md)): Vapi number/assistant provisioning automation, analytics views, billing, and Supabase RLS for multi-tenant isolation.

## Cold outreach (the `/locksmith-outreach` skill)

A **separate subsystem** from the call→SMS pipeline above: a Claude Code skill that sweeps a city for locksmith (or other trade) businesses and writes them into the shared **`leads` table** via the admin API — the call sheet the team works from in the console's Leads tab. **Calling is the primary channel**; Gmail drafts are opt-in (`--email`). Lives in [.claude/skills/locksmith-outreach/SKILL.md](.claude/skills/locksmith-outreach/SKILL.md); email copy in [outreach/templates.md](outreach/templates.md). `outreach/contacted.csv` is the **retired** v1 ledger (gitignored — scraped business PII), imported once by [scripts/import-legacy-leads.py](scripts/import-legacy-leads.py); nothing writes to it now. Built deliberately **lean** (no paid APIs, no LLM personalization, no agent fan-out) — keep it that way unless asked.

Discovery invariants: the sweep runs **query families in rounds and stops only after two consecutive rounds find no new domain** — one pass of a few searches misses most of a city. Suburb/quadrant queries are where the extra volume is. Aggregators (Yelp, YellowPages, BBB, Facebook…) are a source of business *names* only, never a lead's website. A shop with **no website is still a lead** if a directory gives a consistent name + phone. The per-lead `timezone` must carry the IANA zone in brackets (`Mountain Time (America/Edmonton)`) — the console renders the shop's local clock from it so nobody dials at 6am.

Invariants / traps (learned building it):
- **CASL applies to the email channel only** (cold *calling* a published business number is not CASL-governed; a do-not-call request still is). For email: **implied consent is the legal basis** (targets are Canadian): only email an address **conspicuously published on the business's own website**, and record its `email_source_url`. **Never guess `info@domain`** — a guessed address isn't "published" and weakens the CASL footing. Skip sites stating "no unsolicited email."
